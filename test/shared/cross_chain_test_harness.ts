...
export class CrossChainTestHarness {
  static async new(
    ...
    // added by me
    logger("The token contract: " + token.address.toString() + "\n\n" +
       "The TokenBridge: " + bridge.address.toString() + "\n\n" +
       "The TokenPortal: " + tokenPortalAddress + "\n\n" +
       "The underlyingERC20: " + underlyingERC20.address.toString() + "\n\n" +
        "The outbox address:" + outbox.toString());
  ...
  generateClaimSecret(): [Fr, Fr] {
    this.logger("Generating a (randomized) claim secret using pedersen's hash function");
    const secret = Fr.random();
    const secretHash = computeMessageSecretHash(secret);
    //adjusted by me, added the text for secret
    this.logger('Generated random secret: ' + secret  + "\n\n" + 'Generated claim secret: ' + secretHash.toString());
  ...
  async mintTokensOnL1(amount: bigint) {
    ...
    this.logger("Successful minting of " + amount + "to" + this.ethAccount.toString());
  }
